...
/*
 * Send a 301 status: redirect the browser to the given requestURI,
 * but with a '/' appended to it.
 */
static void send301Status(int clntSock, const char *requestURI)
{
    ...
    // Send Location header and format redirection link in HTML in case
        ...
        const char *requestURI, FILE *mdbFp, int mdbSock, int clntSock)
{
    ...
    if (strncmp(requestURI, keyURI, strlen(keyURI)) == 0) {
        ...
        if (Send(clntSock, form) < 0) return statusCode;

        // Read lines from mdb-lookup-server and send them to the
        // browser, formatted in an HTML table.

        char line[1000];
        char *table_header = "<p><table border>";
        if (Send(clntSock, table_header) < 0) return statusCode;
        ...
        for (;;) {
            ...
            if (Send(clntSock, table_row) < 0) return statusCode;
            if (Send(clntSock, line) < 0) return statusCode;
        }

        char *table_footer = "\n</table>\n";
        if (Send(clntSock, table_footer) < 0) return statusCode;
    }
    else {
        // just send the HTML form.
        sendStatusLine(clntSock, statusCode);
        Send(clntSock, "\r\n"); // no headers needed
        if (Send(clntSock, form) < 0) return statusCode;
    }
...
int main(int argc, char *argv[])
{
    ...
    // Save local server info for 301 redirect responses.
